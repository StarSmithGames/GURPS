...
[CustomEditor(typeof(MoverSensor))]
public class MoverEditor : OdinEditor
{
	...
	private void Setup()
	{
		...
		var sensor = (MoverSensor)target;
		var settings = serializedObject.FindProperty("settings").GetValue<MoverSensor.Settings>();

		if (settings.sensorType == Sensor.CastType.RaycastArray)
		{
			Rect _space = GUILayoutUtility.GetRect(GUIContent.none, GUIStyle.none, GUILayout.Height(100));
			...
			Vector3[] _previewPositions = sensor.raycastArrayPreviewPositions;
